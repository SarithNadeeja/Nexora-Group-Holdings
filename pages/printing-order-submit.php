...
$documentPrice = isset($input['documentPrice']) ? (float) $input['documentPrice'] : -1;
...
try {
    ...
    $ins = $pdo->prepare(
        'INSERT INTO printing_orders
            (document_id, document_price, customer_name, customer_phone, customer_email, address_line1, address_line2, city, province, status)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?) RETURNING id'
    );
    $ins->execute([
        $documentId,
    ...
    $lines = [
        '*Nexora Printing - Order request*',
        '',
        '*Order*',
        'Order ID: #' . $orderId,
        'Document ID: #' . $documentId,
        'Price: LKR ' . number_format($documentPrice, 2, '.', ''),
        '',
        '*Customer*',
        'Name: ' . $customerName,
        'Phone: ' . $customerPhone,
