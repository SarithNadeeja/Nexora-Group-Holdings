...
$customRequest = trim((string) ($input['customRequest'] ?? ''));
...
if ($customRequest === '' || $customerName === '' || $customerPhone === '' || $customerEmail === '' || $address1 === '' || $city === '' || $province === '') {
...
try {
    ...
    $ins = $pdo->prepare(
        'INSERT INTO printing_custom_orders
            (custom_request, customer_name, customer_phone, customer_email, address_line1, address_line2, city, province, status)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?) RETURNING id'
    );
    $ins->execute([
        $customRequest,
    ...
    $digits = nexora_whatsapp_printing_order_digits();
    if ($digits === '') {
        http_response_code(422);
    ...
    $lines = [
        '*Nexora Printing - Custom printout request*',
        '',
